...
# =============================================================================
# ALTERNATIVE: Scale-In Logic (Instead of Hard Cooldown)
# =============================================================================

class ScaleInManager:
    """
    Alternative to hard cooldown: Allow re-purchases but reduce position size

    Instead of blocking re-purchases, this scales down position size:
    - First buy:  100% of base size
    - Second buy: 50% of base size
    - Third buy:  25% of base size
    - Fourth buy: 12.5% of base size (and so on)

    This allows flexibility while still managing concentration risk
    """

    def __init__(self, max_positions=3):
        """
        Initialize scale-in manager

        Args:
            max_positions: Maximum number of positions in same ticker
        """
        self.max_positions = max_positions
        self.entry_counts = {}  # {ticker: number_of_entries}
    ...
    def can_buy(self, ticker):
        ...
        Returns:
            bool: True if under max_positions limit
        """
        current_count = self.entry_counts.get(ticker, 0)
        return current_count < self.max_positions

    def get_size_multiplier(self, ticker):
        """
        Get position size multiplier based on existing entries

        Returns:
            float: Multiplier (1.0, 0.5, 0.25, 0.125, etc.)
        """
        current_count = self.entry_counts.get(ticker, 0)

        # Each additional position is half the size of previous
        return 0.5 ** current_count

    def record_buy(self, ticker):
        """Record a new entry"""
        if ticker not in self.entry_counts:
            self.entry_counts[ticker] = 0
        self.entry_counts[ticker] += 1

    def clear(self, ticker):
        """Clear when position fully closed"""
        if ticker in self.entry_counts:
            del self.entry_counts[ticker]

    def get_status(self, ticker):
        """Get current status for ticker"""
        current_count = self.entry_counts.get(ticker, 0)

        return {
            'can_buy': current_count < self.max_positions,
            'entry_count': current_count,
            'max_positions': self.max_positions,
            'next_size_multiplier': 0.5 ** current_count if current_count < self.max_positions else 0
        }